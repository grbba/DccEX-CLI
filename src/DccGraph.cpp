...
auto DccGraph::checkModule(dccexlayout::Module *module) -> int
{
    ...
    DBG("Module[%d]: [%s] using track plan [%s]", module->get_dccid(), module->get_name(), module->get_trackplan());
    ...
    for (auto tp : layout->get_trackplans())
    {
        ...
        {
            INFO("Found track plan [%s] for Module [%s]", tp.get_name(), module->get_name());
            hasTp = true;
            tps.push_back(tp);
...
std::shared_ptr<DccVertex> DccGraph::createVertex(int16_t module, int32_t dccid, Node_t nodeid, TrackElement_t te)
{
    
    std::shared_ptr<DccVertex> v = std::make_shared<DccVertex>(obtainNGid(), module, dccid, nodeid, te);
    ...
    // if the  vector exists just push the vertex into it otherwise create it and the push it
    if (it == tev.end())
    {
    ...
    std::string type;
    switch ( v->getTeType() ) {
        case BUMPER:{type = "Bumper"; break; }
        case TURNOUT:{type = "Turnout"; break;}
        case RAIL:{type = "Rail"; break;}
        case CROSSING:{type = "Crossing"; break;}
        case JUNCTION: {type = "Junction"; break;}
    }

    TRC("Created %s vertices: Module:[%d], TrackElement:[%d], PathId:[%d]", type, v->getModule(), v->getDccid(), v->getNodeid());
    return v;
}
 ...
 */
void DccGraph::buildBumperVertex(int mn, int ten, dccexlayout::Trackelement *te)
{
    DBG("Bumper[%d] %s", te->get_dccid(), te->get_bumper().get()->get_description() == nullptr ? "" : te->get_bumper().get()->get_description().get()->c_str());
    // creates one vertex
    Node_t vid = te->get_bumper().get()->get_path();
    createVertex(mn, ten, vid, te->get_bumper());
}

void DccGraph::buildTurnoutVertex(int mn, int ten, dccexlayout::Trackelement *te)
{
    // creates three vertices
    DBG("Turnout[%d] %s", te->get_dccid(), te->get_turnout().get()->get_description() == nullptr ? "" : te->get_turnout().get()->get_description().get()->c_str());
...
void DccGraph::buildRailVertex(int mn, int ten, dccexlayout::Trackelement *te)
{
    ...
    DBG("Segment[%d] %s", te->get_dccid(), te->get_rail().get()->get_description() == nullptr ? "" : te->get_rail().get()->get_description().get()->c_str());
...
void DccGraph::buildCrossingVertex(int mn, int ten, dccexlayout::Trackelement *te)
{
    ...
    // creates two vertices
    DBG("Crossing [%d] %s", te->get_dccid(), te->get_crossing().get()->get_description() == nullptr ? "" : te->get_crossing().get()->get_description().get()->c_str());
    ...
    else
    {
        ...
        DBG("Create [%d] vertices for the crossing", te->get_crossing().get()->get_path().size());
        for (unsigned long i = 0; i < te->get_crossing().get()->get_path().size(); i++)
        {
        ...
        for (int i = 0; i < 2; i++)
        {
            in = crNodes.at(i);
            out = crNodes.at(i + 2);
            DBG("Adding out [%d] to in [%d] connection", out.get()->getNodeid(), in.get()->getNodeid());
            in.get()->addConnection(out);
            DBG("Adding in [%d] to out [%d] connection", in.get()->getNodeid(), out.get()->getNodeid());
            out.get()->addConnection(in);
        }
        // add slips if any; none -> just a crossing
        if (te->get_crossing().get()->get_slip().size() > 0)
        {
            DBG("Adding [%d] slips to the connections between the vertices of the crossing", te->get_crossing().get()->get_slip().size());
            // loop over all slips
            for (unsigned long i = 0; i < te->get_crossing().get()->get_slip().size(); i++)
            {
                // find the vertex for the first slip point in crNodes based on the index
                unsigned long j = 0;
                for (j = 0; j < te->get_crossing().get()->get_path().size(); j++)
                {
                    if (crNodes.at(j).get()->getNodeid() == te->get_crossing().get()->get_slip().at(i))
                    {
                        in = crNodes.at(j);
                        break;
                    }
                ...
                // find the opposite for the slip point; As the user can define any node as the start/end of the slip look for the direction
                if (j == 0 || j == 2)
                {
                    j++;
                }
...
{
    dccexlayout::Trackplan tp = tps.at(mn - 1); // vector index starts at 0 but we number the modules starting at 1
    ...
    /**
     * @todo error checking on the contents of the layout if not caught before by the validator
     * 
     */
...
void DccGraph::build(dccexlayout::DccExLayout *l)
{
    ...
    // as there may be more trackplans in there as are used by modules collect the used ones in a vector
    INFO("Setting up Modules ... ");
    layout = l;
    for (auto module : layout->get_modules())
    {
        int mn = checkModule(&module); // get back the module number for the last checked module
        if (mn > 0)
        { // if it's 0 we don't have a trackplan for this module
            buildGraph(mn);
        }
    }

    INFO("Setting up Junctions ... ");
    ...
    for (unsigned long i = 0; i < layout->get_junctions().get()->size(); i++)
    {
        ...
        TrackElement_t j = std::make_shared<dccexlayout::Junction>(layout->get_junctions().get()->at(i));
        ...
        auto from = std::get_if<std::shared_ptr<dccexlayout::Junction>>(&j)->get()->get_from();
        auto to = std::get_if<std::shared_ptr<dccexlayout::Junction>>(&j)->get()->get_to();

        TRC("Creating Junction #%d from [%s : %d] to [%s : %d]", i, from.get_from_module(), from.get_path(), to.get_to_module(), to.get_path()); 

        int mn1 = 0;
        int mn2 = 0;
        DccVertexPtr_t in, out;
        bool isJunction1 = false;
        bool isJunction2 = false;

        auto itf = modules.find(from.get_from_module()); // that gives me the name -> find the module number
        if (itf != modules.end()) {
            mn1 = modules.find(from.get_from_module())->second;
            in = createVertex(mn1, ten, from.get_path(), j );
            isJunction1 = true;
        } else {
            ERR("No Module [%s] found for Junction node [%d]", from.get_from_module(), from.get_path()); 
            isJunction1 = false; 
        }

        auto itt = modules.find(to.get_to_module()); // that gives me the name -> find the module number
        if (itt != modules.end()) {
            mn2 = modules.find(to.get_to_module())->second;
            out = createVertex(mn2, ten, to.get_path(), j );
            isJunction2 = true;
        } else {
              ERR("No Module [%s] found for Junction node [%d]", to.get_to_module(),to.get_path() ); 
              isJunction2 = false;
        }
        ...
        // length = length + te->get_rail().get()->get_length(); / do we have to care about the length ??
    }

    // in tps we have now all the trackplans used by the layout
    // and in graph all vertices ready to be combined together into the double vertices
    // there is a double vertice for each of the trackelements; loop over the track element number
    // (ten) and get the vertices out of graph which have the same nodeid eq to the current
    ...
    // for (int32_t i = 1; i <= getNumberOfNodes(); i++)
    {
        ...
        for (auto v : graph)
        {
            ...
            if (vptr.get()->getNodeid() == i)
            {
                ...
                // Tell the vertex which dv it is part of; allows to navigate from one Dv to the next based on the
                // connections of one of the siblings
                vptr.get()->setDv(dv);
        ...
        case 0:
        {
            ...
            WARN("WARNING: No siblings for nodeid [%d]: Node is (maybe intentionally) not connected/skipped in the track plan", i);
            break;
        }
        case 1:
        {
            // End of track i.e. Bumper ommitted most likely
            WARN("WARNING: Missing sibling for nodeid [%d]: Bumper or connection is missing", i);
            break;
        }
        case 2:
        {
        ...
        case 3: {
            ...
            // remove the bumper from the siblings ( costly though but no other solution yet )
            // remove the bumper
            WARN("WARNING: Possible reconfiguration from a bumper to a junction sibling for nodeid [%d]", i);
            for (size_t i = 0; i < siblings->size(); i++)
            {
                DBG("Found sibling #%d of type %s", i, siblings->at(i).get()->getTrackElementType());
        ...
        {
            addSiblings = true;
            WARN("Too many siblings for nodeid [%d]; ignoring the additional siblings ", i);
        }
        }
