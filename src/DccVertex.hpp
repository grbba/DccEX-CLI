...
typedef std::map<std::pair<int64_t, int64_t>, DccDoubleVertexPtr_t>
...
private:
  ...
  int32_t dccid;  // unique id per trackelement ( but not for the vertices of a
                  // trackelement )
  Node_t
      nodeid; // Attn the node id is not unique as the same id will be used in
              // the layout description multiple times to represent connectoins
  Telement teType; // enum indicating the Track element type
  TrackElement_t trackelement;

  std::vector<DccVertexPtr_t>
      connections; // all the other vertices to which this one is connected
  DccDoubleVertexPtr_t dv; // The double vertex wich this vertex is part of

public:
  GraphMarker mark = NOK;
  // Getter/Setter
  void setTrackelement(TrackElement_t value) { trackelement = value; }
  TrackElement_t getTrackelement() { return trackelement; }

  void setTeType(Telement value) { teType = value; }
  Telement getTeType() { return teType; }

  int16_t getModule() { return module; }
  void setModule(int16_t value) { module = value; }

  void setGid(int64_t value) { gid = value; };
  int64_t getGid() { return gid; };

  void setDccid(int32_t value) { dccid = value; };
  int32_t getDccid() { return dccid; }

  int32_t getNodeid() { return nodeid; }
  void setNodeid(int32_t value) { nodeid = value; };

  void addConnection(DccVertexPtr_t v) { connections.push_back(v); }
  std::vector<DccVertexPtr_t> getConnections() { return connections; }

  void setDv(DccDoubleVertexPtr_t value) { dv = value; }
  DccDoubleVertexPtr_t getDv() { return dv; }
  ...
  // Function Methods
  /**
   * @brief Determines the TrackElement type from the trackelement variant
   * attribute; Only for printing purposes
   *
  ...
  DccVertex() = default;
  DccVertex(int64_t gid, int16_t module, int32_t dccid, Node_t nodeid,
            TrackElement_t te);
  ~DccVertex() = default;
};
...
private:
  ...
  std::vector<DccDoubleVertexPtr_t>
      connections; // all double vertices to which the vertex v1 and v2 are
                   // connected

public:
  GraphMarker mark;
  std::vector<DccVertexPtr_t> *getSiblings() { return &siblings; };
  DccVertexPtr_t findSibling(DccVertexPtr_t v);

  std::vector<DccDoubleVertexPtr_t> getConnections() { return connections; }
