 ...
 */
bool findStartVertex(int32_t nodeid, std::vector<DccVertexPtr_t> *start, DccVertexGraph_t *g)
{
    ...
    for (auto vertex : *g)
    {
        ...
        {
            // std::cout << nodeid << " " << vertex.second.get()->getNodeid() << " " << vertex.second.get()->getTeType() << std::endl;

            switch (vertex.second.get()->getTeType())
            {
            case BUMPER: // start from its sibling
            {
                DccVertexPtr_t pb = vertex.second.get()->getDv().get()->findSibling(vertex.second);
                if (pb == nullptr)
                {
                    break;
                }
 ...
 * As we can/shall have trains only on rail segments these are they only reasonable ones 
 */
void DccPathFinder::findAllRail2RailPaths()
{
    ...
    // for each nodeid run the path finder and insert all the pointers of found paths
    // into a nxn matrix ( n be the modeids) and store in (x,y) the poitner to the path allowing to go from x to y
    // providing a second graph allwing to determine ev reverse operatons to get from one segment to another where
    // you can not go directly e.g. in time saver 1 6 R 12 R 15 R 20
}
...
void DccPathFinder::printPathsByNode(Node_t nodeid)
{
    ...
    INFO("[%d] direct paths are available from module [%d] node [%d]", p->size(), m,  n);
    for (auto pa : *p)
    {
        int _pn, _pm;
        std::cout << "start -> ";
...
/**
 * @brief Public: Finds all paths from the start nodeid; the found paths will be stored in paths 
 * 
...
/**
 * @brief Public: Finds all possible paths between two nodes. Start and end nodes only from rail segments are allowed
 * 
...
void DccPathFinder::findAllPaths(int32_t s, int32_t e)
{
    ...
    // report all paths found
    INFO("[%d] direct paths are available from node [%d] to node [%d]", paths.size(), _sn, _en);
    for (auto pa : paths)
    {
        int _pn, _pm;
...
/**
 * @brief Private: Finds all paths from start to end; Called by findAllFGrahs(start , end)
 * 
 * @param path 
 * @param start Vertex pointer to the start node
 * @param end  nodeid of the end point (that this is a rail segment node was checked before in findAllGraphs)
 */
void DccPathFinder::DFS(DccPath_t path, DccVertexPtr_t start, int32_t end)
{
    ...
    if (start.get()->getConnections().size() == 0 || start.get()->getNodeid() == end || (start.get()->getNodeid() == _start && start.get()->mark == OK))
    {
    ...
    for (auto c : start.get()->getConnections())
    {
        ...
        // for each connected vertex of start get the dv of that vertex and look for its sibling
...
/**
 * @brief Private: Recursivley finds all paths starting from a vertex and stores each path in the collection of paths
 * 
 * @param path Vector which contains all the nodes collected until a path is found
 * @param start Pointer to the vertex fro which we shall start. This vertex will be defined by the public function DFS
 */
void DccPathFinder::DFS(DccPath_t path, DccVertexPtr_t start)
{
    ...
    // we are done if there are no more connections
    // or if we find the inital start node again and this one has been visited already ( circle )
    if (start.get()->getConnections().size() == 0 || (start.get()->getNodeid() == _start && start.get()->mark == OK))
    {
