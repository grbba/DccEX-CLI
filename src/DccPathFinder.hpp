...
private:
    ...
    DccNodePaths_t paths;                         // contains all paths found by either findAllPaths or DFS for one run of any find... function
    std::map<Node_t, DccNodePaths_t> allPaths;    // a map of all paths found in a graph <encoded nodeid, vector of all the paths between Rail segments

    void DFS(DccPath_t path, DccVertexPtr_t start);
    void DFS(DccPath_t path, DccVertexPtr_t start, int32_t end);
    void findAllPaths(int32_t start, int32_t end);      // find all paths between two nodes
    void findAllRail2RailPaths();                       // find all paths between any two rail segment nodes
